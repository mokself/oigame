package com.msr.oigame.core.text;

import com.msr.oigame.util.ArrayUtil;
...
public class StrFormatter {
        ...
        for (int argIndex = 0; argIndex < argArray.length; argIndex++) {
            ...
            if (delimIndex > 0 && strPattern.charAt(delimIndex - 1) == StrUtil.C_BACKSLASH) {// 转义符
                if (delimIndex > 1 && strPattern.charAt(delimIndex - 2) == StrUtil.C_BACKSLASH) {// 双转义符
                    ...
                    sbuf.append(StrUtil.str(argArray[argIndex], StandardCharsets.UTF_8));
    ...
    public static String format(CharSequence template, Map<?, ?> map, boolean ignoreNull) {
        ...
        for (Map.Entry<?, ?> entry : map.entrySet()) {
            value = StrUtil.str(entry.getValue(), StandardCharsets.UTF_8);
